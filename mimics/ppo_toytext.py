...
class Args:
    ...
    """Dimension of the embedding layer for discrete observations"""
...
class Agent(nn.Module):
    def __init__(self, envs, embedding_dim=16):
        super().__init__()
        ...
        if isinstance(observation_space, gym.spaces.Discrete):
            ...
            self.embedded = False
            input_size = np.array(observation_space.shape).prod()
...
if __name__ == "__main__":
    ...
    agent = Agent(envs, embedding_dim=args.embedding_dim).to(device)
    ...
    if isinstance(envs.single_observation_space, gym.spaces.Discrete):
        next_obs = torch.tensor(next_obs).long().to(device)
    else:
